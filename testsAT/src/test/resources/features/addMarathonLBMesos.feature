...
Feature: [QATM-100]Adding marathon-lb Default
  ...
  Background: Obtain vault token from bootstrap system
    ...
    Then I run 'grep -Po '"root_token":"(\d*?,|.*?[^\\]")' /stratio_volume/vault_response | awk -F":" '{print $2}' | sed -e 's/^"//' -e 's/"$//'' in the ssh connection and save the value in environment variable 'vaultToken'
  ...
  Scenario: [QATM-100]Add Default marathon-lb
    ...
    And I create file 'config.json' based on 'schemas/marathon-lb-sec-config.json' as 'json' with:
      ...
      | $.marathon-lb.minimumHealthCapacity          | REPLACE | ${MINIMUMHEALTHCAPACITY:-0.5}                                              | number  |
      | $.marathon-lb.maximumOverCapacity            | REPLACE | ${MAXIMUNOVERCAPACITY:-0.2}                                                | number  |
      | $.marathon-lb.name                           | UPDATE  | ${NAME:-marathon-lb-sec}                                                   | n/a     |
      | $.marathon-lb.role                           | UPDATE  | ${ROLE:-slave_public}                                                      | n/a     |
      | $.marathon-lb.ssl-cert                       | UPDATE  | ${SSL_CERT:-""}                                                            | n/a     |
    ...
    When I run 'dcos package install --app --yes --options=/tmp/config.json ${PACKAGE:-marathon-lb-sec}' in the ssh connection
    ...
    Then in less than '300' seconds, checking each '20' seconds, the command output 'dcos task | grep ${NAME:-marathon-lb-sec} | grep R | wc -l' contains '1'
    # Find task-id if from DCOS-CLI
    And in less than '300' seconds, checking each '20' seconds, the command output 'dcos marathon task list ${NAME:-marathon-lb-sec} | grep ${NAME:-marathon-lb-sec} | awk '{print $2}'' contains 'True'
    And I run 'dcos marathon task list ${NAME:-marathon-lb-sec} | awk '{print $5}' | grep ${NAME:-marathon-lb-sec}' in the ssh connection and save the value in environment variable 'marathonTaskId'
